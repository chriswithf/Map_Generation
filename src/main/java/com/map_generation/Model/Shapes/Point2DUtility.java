...
public class Point2DUtility {
    public static double averageDepth(double x, double y, Point2D p1, Point2D p2, Point2D p3) {
        final Vector3D v1 = new Vector3D(p1.x - p2.x, p1.y - p2.y, p1.depth - p2.depth);

        final Vector3D v2 = new Vector3D(p1.x - p3.x, p1.y - p3.y, p1.depth - p3.depth);

        final Vector3D cross = Vector3DUtility.cross(v1, v2);

        return (1 / cross.z) * (cross.x * p1.x + cross.y * p1.y + cross.z * p1.depth - cross.x * x - cross.y * y);
    }
